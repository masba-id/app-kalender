<!DOCTYPE HTML>
<html>
    <head>
        <title>App Kalender &mdash; Created by BAGOS ANGGARA</title>
        <!-- Bootstrap core CSS -->
        <link href="<?=base_url('assets/css/bootstrap.min.css')?>" rel="stylesheet">
        <!-- jQuery UI core CSS --> 
        <link href="//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css" rel="stylesheet">
        <!-- Custom CSS -->
        <link href="<?=base_url('assets/css/custom.css')?>" rel="stylesheet">
        <!-- Custom jQuery UI CSS -->
        <link href="<?=base_url('assets/css/custom-jquery-ui.css')?>" rel="stylesheet">
        
        <!-- jQuery 2.1.4 Core JavaScript -->
        <script src="<?=base_url('assets/js/jquery.min.js')?>"></script>

        <!-- jQuery 1.12.4 Core JavaScript -->
        <!-- <script src="https://code.jquery.com/jquery-1.12.4.js"></script> -->

        <!-- Bootstrap core JavaScript -->
        <script src="<?=base_url('assets/js/bootstrap.min.js')?>"></script>
        <!-- jQuery UI core JavaScript -->
        <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
        <!-- jQuery UI Dialog Options -->
        <!-- <script src="<?=base_url('assets/js/jquery-ui.dialogOptions.js')?>"></script> -->
        <!-- Custom jQuery UI JavaScript -->
        <!-- <script src="<?=base_url('assets/js/custom-jquery-ui.js')?>"></script> -->
        <script>
            var img_loader = '<img src="data:image/gif;base64,">';
        </script>
    </head>
    <body>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h1 class="text-center app-title"><i class="glyphicon glyphicon-calendar" style="padding-right: 10px;"></i>Kalender <?=date('Y')?></h1>
                    
                    <hr>
                    <div class="form-group" id="btn-reload-wrapper">
                        <div class="btn btn-info" id="btn-reload">
                            <strong><i class="glyphicon glyphicon-refresh"></i> Reload Kalender</strong>
                        </div>
                    </div>
                    <div class="form-group">
                        <input type="text" class="search" name="search" placeholder="Cari Nama...">
                    </div>
                    <div class="form-group" id="btn-back-wrapper" style="display:none;">
                        <div class="btn btn-default" id="btn-back">
                            <strong><i class="glyphicon glyphicon-arrow-left"></i> Kembali</strong>
                        </div>
                    </div>

                    <div class="row" id="loader-wrapper" style="display:none;">
                        <div class="col-md-12">
                            <img src="data:image/gif;base64,"> Please wait...
                        </div>
                    </div>

                    <div class="row" id="search-results-wrapper" style="display:none;">
                        <div class="col-md-12"></div>
                    </div>
                    
                    <?php echo $default_wrapper; ?>

                    <hr>
                    <div class="row" id="keterangan-wrapper">
                        <div class="col-md-12">
                            <h5><b>Keterangan:</b></h5>
                            <div class="tag-table tabel-keterangan" style="margin:0;">
                                <div class="tag-tbody">
                                    <div class="tag-tr">
                                        <div class="tag-td keterangan same-width holiday">Hari Libur</div>
                                    </div>
                                    <div class="tag-tr">
                                        <div class="tag-td keterangan same-width birthday">Hari Ulang Tahun</div>
                                    </div>
                                    <div class="tag-tr">
                                        <div class="tag-td keterangan same-width currentday">Hari Ini (<?=date('d-m-Y')?>)</div>
                                    </div>
                                    <div class="tag-tr">
                                        <div class="tag-td keterangan same-width specialdate">HUT RI</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    
                </div>
            </div>
        </div>
        
        <footer style="margin: 10px 0">
            <!--Copyright-->
            <div class="text-center">
                <p>© 2018</p>
                <h5 class="credit-author">BAGOS ANGGARA</h5>
            </div>
            <!--/.Copyright-->
        </footer>

        <?php $this->load->view('calendar-modal'); ?>
        <?php $this->load->view('add-form-modal'); ?>
		
        <!-- jQuery Mask -->
        <script src="<?=base_url('assets/js/jquery.mask.min.js')?>"></script>
        <!-- Custom JavaScript -->
        <script src="<?=base_url('assets/js/custom.js')?>"></script>
    </body>
</html>